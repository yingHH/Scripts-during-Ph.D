...
def homogenes(sp_idx, gffs, homo_db=HOMO_DB):
    ...
    #print(splst.search(9031))

    sp_ids = [convert_sp_idx_to_id(splst, i) for i in sp_idx]
    #print(sp_ids, type(sp_ids))
    #print(','.join(sp_ids))
    #print([splst.search(i) for i in sp_ids])
    sp_names = [splst.search(i) for i in sp_ids]
    #print(sp_names)

    print('<!!! Note: gffs order must be consistent with sp_idx order !!!> ')
    gff_sp_dict = {g:s for g, s in zip(gffs, sp_names)}
    print(
        '<Here is input species gff3 file dict, please check it>: \n\t',
        '\n\t'.join(['"{}": "{}"'.format(s,g) for g, s in zip(gffs, sp_names)])
    )
    ...
    # get all homologous genes in database
    homogenes_df.columns = new_col
    homognames_df = homogenes_df[[i for i in homogenes_df.columns if i.startswith('gene_name')]]
    # gat all genes with same name across gff3 files
    samegnames_df = merge_same_gnames_in_refseq(gffs)
    samegnames_df['SP_GFF'] = samegnames_df['SP_GFF'].replace(gff_sp_dict)
    samegnames_df.set_index(['SP_GFF'], drop=True, inplace=True)
    samegnames_df = samegnames_df.T
    #print(samegnames_df.head())
    samegnames_df.columns = ['gene_name_' + sp for sp in samegnames_df.columns]

    #print(homognames_df.head(), samegnames_df.head())
    homo_df = pd.concat([homognames_df, samegnames_df], axis=0)

    return homo_df.drop_duplicates()
